        ...
        <div class="row">
            <div class="col-md-8 m-auto">
              ...
              <form action={{route('doctors.update',$doctor->id)}} method="post" enctype="multipart/form-data" class="text-center">
                ...
                @enderror
                 <select name="specializations_id" id="specializations_id" class="form-control mt-3" value={{$doctor->specializations_id}}>
                    <option value="">Choose Specialization</option>

                    @foreach ($specializations as $item)
                    <option value={{$item->id}} {{$doctor->specializations_id == $item->id ? 'selected' : ''}}>{{$item->name}}</option>
                    @endforeach
